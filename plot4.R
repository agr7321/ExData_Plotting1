...
par (mfrow=c(2,2))
with (pf_final, {
  plot(pf_final$Time, pf_final$Global_active_power, pch=NA, ylab="Global Active Power (kilowatts)", xlab=NA, xaxt='n')
  lines (pf_final$Time, pf_final$Global_active_power)
  axis(1, at=c(0, 86400, 172800), labels=c("Thu", "Fri", "Sat"))
  plot (pf_final$Time, pf_final$Voltage, ylab="Voltage", pch=NA, xlab="datetime", xaxt='n')
  lines (pf_final$Time, pf_final$Voltage)
  axis (1, at=c(0, 86400, 172800), labels=c("Thu", "Fri", "Sat"))
  ...
  plot(pf_final$Time,pf_final$Global_reactive_power, ylab="Global_reactive_power", xlab="datetime", xaxt='n', pch=NA)
  lines(pf_final$Time, pf_final$Global_reactive_power)
  axis(1, at=c(0, 86400, 172800), labels=c("Thu", "Fri", "Sat"))   
})
dev.off()